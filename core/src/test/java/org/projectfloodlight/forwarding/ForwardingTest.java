...
package org.projectfloodlight.forwarding;
...
import org.projectfloodlight.forwarding.Forwarding;
...
public class ForwardingTest extends FloodlightTestCase {
    ...
    public void setupSwMock() {
        ...
        expect(sw1.getPort(EasyMock.anyShort())).andReturn(null).anyTimes();
        ...
        expect(sw2.getPort(EasyMock.anyShort())).andReturn(null).anyTimes();
        ...
        mockFloodlightProvider.setSwitches(ImmutableMap.of(1L, sw1, 2L, sw2));
    }
    ...
    public void testFlowModDampening() throws Exception {
        ...
        setupSwMock();
        sw1.writeThrottled(eq(fm1), eq(cntx));
        expectLastCall().once();
        sw1.writeThrottled(eq(packetOut), eq(cntx));
