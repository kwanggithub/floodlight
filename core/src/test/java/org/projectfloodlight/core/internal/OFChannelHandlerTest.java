...
import org.projectfloodlight.core.internal.OFChannelHandler;
import org.projectfloodlight.core.internal.SwitchStateException;
...
import org.projectfloodlight.debugcounter.DebugCounter;
...
public class OFChannelHandlerTest {
    ...
    private MockBigDBService bigDBService;
     ...
     * @throws Exception 
     */
    private void setupMockBigDB(MockConfig cfg) throws Exception {
        bigDBService = new MockBigDBService();
        bigDBService.addModuleSchema("floodlight", "2012-10-22");
        bigDBService.init(null);
        bigDBService.startUp(null);
        
        if (cfg.isPresent && cfg.dpid != null) {
            Query q = Query.builder().setBasePath("/core/switch").getQuery();
            String data = 
                    "{\"dpid\": \"" + cfg.dpid + 
                    "\", \"core-switch\": " + cfg.isCoreSwitch + "}";
            InputStream input = 
                    new ByteArrayInputStream(data.getBytes("UTF-8"));

            bigDBService.getControllerTreespace()
                .insertData(q, Treespace.DataFormat.JSON,
     ...
     */
    public void doMoveToWaitInitialRole(MockConfig cfg)
            throws Exception {
        ...
        expect(controller.getBigDBService())
                .andReturn(bigDBService).atLeastOnce();
