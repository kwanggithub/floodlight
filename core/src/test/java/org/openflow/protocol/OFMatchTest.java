...
public class OFMatchTest extends TestCase {
    public void testFromString() {
        ...
        tester.fromString("nw-src=1.1.2.3");
        ...
        tester = new OFMatch();
        tester.fromString("dl-vlan=0xffff");
        TestCase.assertEquals(correct, tester);
        }

    public void testToString() {
        OFMatch match = new OFMatch();
        match.fromString("nw-dst=3.4.5.6/8");
        TestCase.assertEquals(8, match.getNetworkDestinationMaskLen());
        String correct = "OFMatch[nw-dst=3.0.0.0/8, wildcards = [in_port|dl_vlan|dl_src|dl_dst|dl_type|nw_proto|tp_src|tp_dst|dl_vlan_pcp|nw_src|nw_tos]]";
        String tester = match.toString();
        TestCase.assertEquals(correct, tester);
        tester = "OFMatch[dl-type=35020]";
        correct = "OFMatch[dl-type=0x88cc, wildcards = [in_port|dl_vlan|dl_src|dl_dst|nw_proto|tp_src|tp_dst|dl_vlan_pcp|nw_src|nw_dst|nw_tos]]";
        match = new OFMatch();
        match.fromString(tester);
        TestCase.assertEquals(correct, match.toString());
        OFMatch match2 = new OFMatch();
        match2.fromString("OFMatch[dl-type=0x88cc]");
