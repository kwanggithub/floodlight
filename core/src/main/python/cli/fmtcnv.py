...
def decode_flow_cookie(i, data=None):
    ...
    try:
        convert_to_interger = int(i)
    except:
        return # assume this has already been converted.
...
def formatter_to_alias_update(formatter, update):
    ...
    if name in ['print_host_attachment_point',
                'print_bvs_physical_interface_id',
...
def print_bvs_count_dict(i, data=None):
    output = ""
    more = 0
    for bvs in i:
        # possibly bvs:refs instead?
        #item = '%s:%s ' % (bvs, i[bvs])
        item = '%s ' % bvs
        if len(output) > 40:
            more += 1
...
def print_bvs_physical_interface_id(i, data=None):
    return i.split('|')[2]
