...
def bigdb_bandwidth(value, schema, detail, bigdb):
    if value == 0:
        return '-'
...
def bigdb_bandwidth_bytes_per_sec(value, schema, detail, bigdb):
    if value == 0:
        return '-'
...
def uptime_millisec_formatter(value, schema, detail, bigdb):
...
def bigdb_switch_port_feature(value, schema, detail, bigdb):
    if value == 0:
        return '-'
...
def bigdb_switch_port_flags(value, schema, detail, bigdb):
...
#
def bigdb_of_counter64(value, schema, detail, bigdb):
    if value == -1:
        return 'N/A'
    return value
...
def bigdb_ip_address(value, schema, detail, bigdb):
    # mask types should not be ip-addresses. BSC-3925
    if schema['name'].find('mask') != -1:
        return value

    nice_value = midw.ip_address_to_controller_name(value, bigdb)
    if nice_value == None:
        return value
    if detail == 'details':
