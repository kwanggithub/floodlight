...
# submode_data_adjustment_registry is the dictionary of registered callback to 
# association additional data items for submode commands.  For example, the bigtap submode
# command 'bigtap policy foo' is has an 'optional' rbac-permission parameter,
# which isn't really optional, but when the command is executed, it may be 
# possible to identify the value of the additional parameter required to perform
# the create.   When its not possible, the create_helper can then throw an
# exception with a application specific message.
submode_data_adjustment_registry = {}
...
#
# associate command names with features, so that when help is provided,
# command names which have associated features are validated to determine
# whether the feature is enabled or not.
command_name_feature = {}
command_name_non_feature = []

#
# associate command names with rbac groups, so that when help is provided,
# command names which have associated rbac-group requirements are validated
# to determine whether the user has the group associated or not.
command_name_rbac_group = {}

#
# command_dict is indexed by mode,
command_dict = {}

#
# command_nested_dict is indexed by mode, and contains commands configured with
# a trailing '*', for example 'mode' : 'config-*', intended to mean "all nested
# submodes after config-'.  This can't be computed until all submodes are known,
# which means all the command describption must be configured.
command_nested_dict = {}
...
def commands_for_submode(prefix):
    """
    Return all the matching commands.  Only for the submode
    which is passed in.

    Be sensitive to features, be sensitive to the prefix.
    For commands which have a first keyword which is a pattern,
    use the command description's completio
    """
...
def command_dict_minus_lec():
    ...
    return [x for x in command_dict.keys()
...
def show_command_submode_tree_recurse(mode_dict, level):
    if len(mode_dict):
        for d in mode_dict.keys():
            ...
            items = [x if _is_string(x) else x.pattern for x in command_dict.get(index, '')]
...
def command_dict_append(command, mode, name):
    ...
    command_dict[mode].append(name)


def command_nested_dict_append(command, mode, name):
    if not mode in command_nested_dict:
        command_nested_dict[mode] = []
    if type(name) == dict:
        # this is a regular expression name, add the re.
        if not 'pattern' in name:
            print '%s: missing pattern' % command['self']
        name['re'] = re.compile(name['pattern'])
    if not name in command_nested_dict[mode]:
        command_nested_dict[mode].append(name)
...
def add_command_using_dict(command_description, value):
    ...
    # record any command name which is strictly associated with
    # a feature, and save the feature association.  If, however,
    # a variation of the command appears which has no feature associated,
    # remove any previous feature associations.
    feature = value.get('feature')
    if feature and name not in ['show']: 
        if not name in command_name_non_feature:
            if not name in command_name_feature:
                command_name_feature[name] = []
            if not feature in command_name_feature[name]:
                command_name_feature[name].append(feature)
    if not feature and name in command_name_non_feature:
        if not name in command_name_non_feature:
            command_name_non_feature[name] = []
        if not name in command_name_non_feature:
            command_name_non_feature.append(name)
        if name in command_name_feature[name]:
            del command_name_feature[name]

    # rbac
    rbac_group = value.get('rbac-group')
    if rbac_group:
        if not name in command_name_rbac_group:
            command_name_rbac_group[name] = []
        if type(rbac_group) != list:
            rbac_group = [rbac_group]
        for rg in rbac_group:
            if not rg in command_name_rbac_group[name]:
                command_name_rbac_group[name].append(rg)
    #
    # populate bigsh.controller_dict based on the modes described for this command
    #
    if 'mode' in value:
        if _is_list(mode):
            for m in mode:
...
def add_submode_data_adjustment(name, handler_proc):
    submode_data_adjustment_registry[name] = handler_proc

def submode_adjustor_invoke(create_args, path, update, data_dict, action):
    create_type = type(create_args)
    if create_type == str:
        (submode_data_adjustment_registry[create_args])(path, update, data_dict, action)
    elif create_type == list:
        for item in list:
            submode_adjustor_invoke(item, path, data_dict)
...
def column_header(path, default = None):  # bigdb style
    return bigsh.bigdb.column_header.get(path, default)
...
def is_command_feature_active(command, features):
    # note: call from bigcli and from handle_command,  however
    # when called from bigcli, command is a string (command name),
    # and when called from handle_command, command is the command
    # description dict.
    if not features:
    ...
    if features:
        if not _is_list(features):
            features = (features,)
        for associated_feature in features:
            if feature.feature_enabled(associated_feature):
                return True
        return False
...
class CommandHandler(object):
    ...
    def get_matching_commands(self, command_word, is_no_command, command_list):
        ...
        3) The command may have RBAC associations
...
#                    match[0] += words_matched
    ...
    def handle_one_command(self, command, prefix_match, words):
        ...
        if command.get('submode-data'):
            submode_data = command.get('submode-data')
            # this ought to be moved outside of command.py
            if type(submode_data) == dict:
                # submode_data is indexed by the mode name,
                # and the associated value is the name of the field
                # in data to set.
                # provide several mappings in submode_data:
                #   mode_name : set-field-name  (simple mode values)
                #   mode_name : field_wanted    (mode value is dict, extract)
                #   path_name : set_path        (mode value is dict, set specific)
                for mode in bigsh.run.finder.mode_stack.mode_stack:
                    obj_id = mode['obj']
                    if mode['mode_name'] in submode_data:
                        if type(obj_id) == dict:
                            # peek inside the dict.
                            field_wanted = submode_data[mode['mode_name']]
                            if field_wanted in obj_id:
                                arg_data[field_wanted] = obj_id[field_wanted]
                        elif obj_id != '':
                            arg_data[submode_data[mode['mode_name']]] = obj_id
                        # what about a list type check?
                    elif type(obj_id) == dict:
                        for (n,v) in submode_data.items():
                            if n in obj_id:
                                arg_data[v] = obj_id[n]
                #
                # provide some additional configuration of data
                # items based on some specific well known names.
                # $cached-user-name <- bigdb.cached_user_name
                #
                for (n, v) in submode_data.items():
                    # for example: $name : core/aaa/local-user/user-name
                    #  $cached-user-name : core/aaa/local-user/user-name
                    if n[0] == '$':
                        if n == '$cached-user-name':
                            if bigsh.bigdb.cached_user_name:
                                arg_data[v] = bigsh.bigdb.cached_user_name
    ...
    def handle_command(self, words, text=None):
        ...
        try:
            if len(words) > 0:
                ...
                self.handle_unspecified_command()
            result = self.handle_command_results()
            if result == None:
                pass
            elif type(result) == str:
                yield result
            else: # assume a generator, or a list
                for item in result:
                    yield item
            # result = self.handle_command_results()
        except Exception, e:
    ...
    def handle_command_results(self):
        ...
        if len(self.matches) == 0:
            ...
            if accumulated_error_messages == []:
                raise error.CommandSyntaxError('no matching commands')
        ...
        if debug.description():   # debug details
        ...
        if self.is_no_command:
            ...
            any_removed = False
            if debug.description():
                print 'handle_command_results: check mode stack for deleted objects'

            for index in reversed(range(len(bigsh.run.finder.mode_stack.mode_stack))):
                mode = bigsh.run.finder.mode_stack.mode_stack[index]
                if mode['obj_type'] == None and mode['path'] != None:
                    object = mode['obj']
                    # mode['obj'] is typically a dictionary.
                    if type(object) != dict:
                        keys = bigdb.search_keys[mode['path']]
                        if len(keys) > 1:
                            print 'Multiple keys for path:', mode['path'], keys
                        else:
                            object = { keys[0] : object }
                    if debug.description():
                        print 'handle_command_results: deleted object?', \
                              mode['path'], object
                    if not bigdb.exists(mode['path'], object, max_depth = index):
                        # pop this item
                        if debug.description() or debug.cli():
                            print 'NO: BIGDB LOST OBJECT ', mode.get('obj_type'), mode.get('obj')
                        del bigsh.run.finder.mode_stack.mode_stack[index]
                        any_removed = True
                elif mi.obj_type_in_use_as_related_config_type(mode.get('obj_type')):
                    if debug.description():
                        print 'CONFIG TYPE:', \
                              mode.get('obj_type'), \
                ...
                if mode.get('obj_type') and mode.get('obj'):
                    try:
                        bigsh.get_object_from_store(mode['obj_type'], mode['obj'])
                    except:
                        # pop this item
                        if debug.description():
            ...
            if any_removed:
                bigsh.warning("submode exited due to deleted object")
                bigsh.update_prompt()
...
class CommandCompleter(CommandHandler):
    ...
    def get_default_value(self, arg):
        ...
        if self.is_no_command and default is None and 'field' in arg:
            # use the path and field to determine a default value.
            print 'CommandCompleter.get_default_value'
        return default
    ...
    def handle_incomplete_command(self, arg, arg_scopes, arg_data, fields,
        ...
        # provide a clue whether this command will change modes.
        mode_switch_clue = ''
        command_mode = command.get('mode')
        if command_mode and command_mode[-1] == '*':
            command_mode = command_mode[:-1]
        if bigsh.run.finder.mode_stack.current_mode() != command_mode:
            # need a predicate is_command_config_submode()
            if command.get('command-type') == 'config-submode':
                new_submode = command.get('submode-name')
                if new_submode:
                    new_submode = new_submode.replace('config-','')
                    mode_switch_clue = ' (move to mode %s)' % new_submode
                else:
                    mode_switch_clue = ' (of mode %s)' % command_mode

            else:
                mode_switch_clue = ' (from mode %s)' % command_mode

        if tag is not None and not parsed_tag:
            if tag.lower().startswith(lower_text):
                if completions == None:
                    completions = {}
                completions[tag + ' '] = self.complete_help(arg_scopes) + mode_switch_clue
        else:
            token = arg.get('token')
            ...
            if token is not None:
                if token.lower().startswith(lower_text):
                    if completions == None:
                        completions = {}
                    completions[token + ' '] = self.complete_help(arg_scopes) + mode_switch_clue
            elif type_name == 'enum':
                enum_obj = arg
            elif typedef:
            ...
            if enum_obj:
                ...
                default_value = self.get_default_value(arg)
                if default_value == None:
                    schema_path = bigsh.run.finder.mode_stack.get_current_mode_path()
                    if schema_path and arg.get('field'):
                        print _line(), schema_path, arg.get('field')
                        print 'DV', bigsh.bigdb.path_default.get(this_path)
                        default_value = bigsh.bigdb.path_default.get(this_path)
                if enum_values:
                    ...
                    for enum_value in enum_values:
                        if enum_value.lower().startswith(lower_text):
                            dv = ''
                            if default_value and default_value == enum_value:
                                dv = ' (default)'
                            completions[enum_value + ' '] = \
                                   self.complete_help(arg_scopes) + dv + mode_switch_clue

            if completions == None or len(completions) == 0 or enum_obj:
                # Check to see if there's a custom completion proc
                completion_proc = _lookup_in_scopes('completion', arg_scopes)
                if completion_proc:
                    if completions == None:
                        completions = {}
                    curr_path   = bigsh.run.finder.mode_stack.get_current_mode_path()
                    curr_obj_id = bigsh.run.finder.mode_stack.get_current_mode_obj()
                    command_mode = command.get('mode')
                    if command_mode and command_mode[-1] == '*':
                        command_mode = command_mode[:-1]
                    if command_mode and bigsh.run.finder.mode_stack.current_mode() != command_mode:
                        # this is completing a different item on the stack.
                        # XXX needs better api's here.
                        found_in_mode_stack = False
                        for x in bigsh.run.finder.mode_stack.mode_stack:
                    ...
                    invocation_scope = {
                        ...
                        'is-no-command': self.is_no_command,
                        'mode' : command_mode,
                        'mode-switch-clue' : mode_switch_clue,
                        'current-mode-path': curr_path,
                        'current-mode-obj-id': curr_obj_id,
                    }
                    arg_scopes.insert(0, invocation_scope)
                    if debug.description(): # for deubugging, print command name
            ...
            if (completions == None or len(completions) == 0 or
              self.meta_completion) and not self.text and not enum_obj:
                ...
                if completion_text is not None:
                    ...
                    bigdb = bigsh.bigdb
                    if bigdb.enabled and completion_text != None \
                      and completion_text.find('/') != -1:
                        completion_text = bigdb.column_header.get(completion_text,
                                                                  completion_text)
                    ...
                    if syntax_help == None:
                        ...
                        if typedef and type_name != 'enum':
                            ...
                            if syntax_help == None:
                                # the additional space prevents the user from
                                # typing '<' to select this entry.
                    ...
                    if completion_text and syntax_help:
                        ...
                        completions[(completion_text, syntax_help)] = syntax_help + mode_switch_clue
    ...
    def handle_complete_command(self, prefix_matches, scopes,
        ...
        if not self.text:
            ...
            if short_help == None:
                short_help = ''
                if debug.description():
                    short_help = "Missing 'short-help' for %s" % command['self']

            self.completions[(' <cr> ', '<cr>' )] = short_help
    ...
    def print_completions(self, completions):
        ...
        bigsh.print_completion_help(bigsh.choices_text_builder(meta_completions))


    def handle_command_results(self):
        if self.meta_completion == True:
            # two-column display mode
            bigsh.completion_print = False
            ...
            bigsh.print_completion_help(''.join(text))
        ...
        if len(completions) == 1:
            ...
            if isinstance(completion, tuple):
                ...
                if self.meta_completion == False:
                    bigsh.print_completion_help(help_text)
                completions = None
...
def do_command_completion(words, text):
    ...
    if type(completions) == list or type(completions) == dict:
        return completions
    return [x for x in completions] # generator
...
def validate_type(type_name, value, arg_data, scopes, command):
    """
...
def validate_argument(arg, value, arg_data, scopes, command):
    ...
    if type_name:
        if type_name == 'enum':
            ...
            type_result = validate_type(type_name, value, arg_data, scopes, command)
    else:
        ...
        if base_type_name:
            ...
            if base_typedef:
                ...
                if validation:
                    scopes = [{'typedef': arg, 'value': value, 'data' : arg_data}] + scopes
...
class CommandSyntaxer(CommandHandler):
    ...
    def handle_command_results(self):
        ...
        (term_cols, term_rows) = bigsh.pp.get_terminal_size()
    ...
    for command in command_registry:
        if type(command['name']) == dict:
            if command['name']['title'] == word:
                if 'short-help' in command:
                    short_help.append(command['short-help'])
                if 'all-help' in command:
                    all_help.append(command['all-help'])
        
    if len(short_help) == 1:
        return short_help[0]
    if len(short_help) == 0:
        return None # perhaps return all_help[0]?
    if all_help:
        if len(all_help) > 1:
            if debug.description():
                print 'multiple all-help for', word, all_help

        return all_help[0]
    if debug.description():
        print 'all-help needed for', word
    return short_help[0]
...
def get_command_short_help(word):
    ...
    if len(matching_commands) >= 1:
        short_help = []
        all_help = []
        for (command, exact) in matching_commands:
            if 'short-help' in command:
                short_help.append(command['short-help'])
            if 'all-help' in command:
                all_help.append(command['all-help'])
...
class CommandDocumentor(CommandHandler):
    ...
    def __init__(self, header, format = None):
        ...
        self.words = None
        self.format = format
        (self.term_cols, self.term_rows) = term.get_terminal_size()


    def add_docs(self, name, value):
        if name in bsn_constants.reserved_words and value == None:
        ...
        if len(self.commands) == 1:
            ...
            for last_doc in self.docs_stack:
                if command['self'] in last_doc:
                    (keyword, last_doc_item) = last_doc[command['self']]
                    text = doc.get_text(self, last_doc_item)
    ...
    def handle_command_results(self):
        ...
        if len(self.commands) > 10: # 10 is a magic number,
            help_lines.append('Help: Too many commands match (%d), '
                              ...
                              len(self.commands))
            help_lines.append(bigsh.choices_text_builder(self.docs))
            return '\n'.join(help_lines)
...
def command_submode_dictionary(modes = None):
    if modes == None:
        modes = command_dict.keys() + command_nested_dict.keys()
    #
...
def get_bigdoc(words):
    if len(words):
        handler = CommandDocumentor(header = None, format = 'bigdoc')
        for word in words:
            for c in command_registry:
                if word == c['self']:
            ...
            for c in command_registry:
                if mode == c['mode']:
                    handler = CommandDocumentor(header = '\n\n\n ---- MODE ' + mode,
                                                format = 'bigdoc')
                    handler.add_command(c)
                    bigdoc.append(handler.handle_command_results())
...
def get_bigwiki(words):
    ...
    intro_path = os.path.join(bigsh.doc_dir, 'introduction')
    introductionfile=open(intro_path)
    ...
    str1='\nh2. Show Commands'
    bigwikifile.write(str1)
    exclude_modes = ['config-internal'] #prune out all internal commands

    # order all the command based on the associated path, some 'show' commands
    # may not have associated path's at the top level,  perhaps listing these
    # and suggesting a path be associated for documentation ordering purposes
    # may be a good idea.
    show_commands = {}
    for c in command_registry:
        if c['name'] == 'show' and c['mode'] not in exclude_modes:
            path = c.get('path')
            command_self = c.get('self')
            if not path in show_commands:
                show_commands[path] = []
            show_commands[path].append(c)
    
    # The 'application' path comes before 'core', perhaps the order ought
    # be up modified to move 'core' before these other components

    for path in sorted(show_commands):
        for c in show_commands[path]:
            # Need the keyword(s) for the command.
...
class CommandPermutor(CommandHandler):
    ...
    # -- generate permute output, 
    # -- generate qualify output,
    # -- generate command choices based on (known) field's values
    #
    # What to do about the 'skip'?  What about 'skip' vs field_values?
    # How are the config commands different from other commands

    def __init__(self, qualify = False, field_values = None):
        """
        @param qualify Generate qualify version of the permutations
        @param field_values Generate instance of command using these fields
        """
        super(CommandPermutor, self).__init__()
        self.commands = []
        self.collect = []

        # intended to be boolean, but any predicate type will do.
        self.qualify = qualify

        # field_values is intended to be a dictionary of name:value pairs
        # for field:value replacement.  the goal is to generate complete
        # command populated with the field's values.  when a command requires
        # a field which isn't included in field_values, that command
        # variation must be skipped, and when the command variation is 
        # about to be collected if field_values still remain, the variation
        # is also discarded. 
        if (field_values != None) and type(field_values) != dict:
            print 'CommandPermutor: parameter field_values not dict: %s' % type(field_values)
        else:
            self.field_values = field_values

        self.path = None
        self.path_other = None


    def collect_complete_command(self, line, field_values):
        if field_values != None:
            if len(field_values) != 0:
                # the command can't be added since not all fields were consumed.
                # (the command and the field_values could be added as a tuple)
                return
        if self.qualify:
    ...
    def permute_item(self, item, line, next_args, field_values):
        """
        ...
        if type(item) == str:
            line.append(item)
            self.permute_in_order(next_args[0], line, next_args[1:], field_values)
            return
        ...
        if skip == 'skip' and (field_values != None):
            skip == None
        ...
        if skip != 'skip' and optional:
            if len(next_args):
                ...
                if fv:
                    fv = dict(fv)
                self.permute_in_order(None, line, next_args, fv)
            else:
                self.collect_complete_command(line, field_values)

        choices = item.get('choices')
        if choices:
            self.permute_choices(choices, list(line), next_args, field_values)
            return
        ...
        self.path_other = item.get('other-path')

        if args:
            if type(args) == dict:
                self.permute_item(args, line, next_args, field_values)
            elif type(args) == tuple:
                self.permute_in_order(args, list(line), next_args, field_values)
            else:
            ...
            line.append(token)
            data = item.get('data')
            if data and (field_values != None):
                # verify request item(s) are configured
                for (n,v) in data.items():
                    if not n in field_values:
                        if debug.description():
                            print 'TOKEN REQ CONF MISSING', n
                        return
                    if str(field_values[n]) != str(v):
                        if debug.description():
                            print 'TOKEN REQ DATA ITEM DIFFERS', n, v, field_values[n]
                        return
                # subtract any field values directly set.
                field_values = dict([[n,v] for (n,v) in field_values.items()
                                     if not n in data or data[n] != v])
                if debug.description():
                    print 'TOKEN ITEM REMOVED', data, field_values
        elif field:
            data = item.get('data')
            if data and (field_values != None):
                # verify request item(s) are configured
                for (n,v) in data.items():
                    if not n in field_values:
                        if debug.description():
                            print 'DATA REQ CONF MISSING', n
                        return
                    if str(field_values[n]) != str(v):
                        if debug.description():
                            print 'DATA REQ DATA ITEM DIFFERS', n, v, ield_values[n]
                        return
                # subtract any field values directly set.
                field_values = dict([[n,v] for (n,v) in field_values.items()
                                     if not n in data])
                if debug.description():
                    print 'DATA ITEM REMOVED', data, field_values
            ...
            if item_type == 'enum':
                ...
                if values:
                    ...
                    if type(values) == str:
                        values = [values]
                    if field_values != None: # deal with command generator request
                        if not field in field_values:
                            return
                        requested_value = str(field_values[field])
                        if type(values) == dict:
                            # By using str(v) below, boolean types
                            # match up against the requested value.
                            invert_dict = dict([[str(v),n] for (n,v) in values.items()])
                            if requested_value in invert_dict:
                                values = [utif.quote_string(str(invert_dict[requested_value]))]
                                del field_values[field]
                            else:
                                # attempt to use the 'no' variation of the
                                # command.  More work to handle  other similar ases
                                # what if the command doesn't support 'no'
                                if field_values[field] == False and 'True' in invert_dict:
                                    line.insert(0, 'no')
                                    values = [utif.quote_string(str(invert_dict['True']))]
                                    del field_values[field]
                        elif str(requested_value) in values:
                            values = [utif.quote_string(str(requested_value))]
                            del field_values[field]
                ...
                if tag:
                    line.append(tag)
                field_string = self.field_to_generator(field, field_values)
                if field_string == None:
                    return
                line.append(field_string)
        ...
        if len(next_args):
            self.permute_in_order(None, line, next_args, field_values)
        else:
            self.collect_complete_command(line, field_values)
        

    def permute_choices(self, choices, line, next_args, field_values):
        """
        ...
        for pick in choices:
            if type(pick) == dict:
                new_line = list(line)
                permute_field_values = None
                if field_values != None:
                    field_values = dict(field_values)
                    permute_field_values = dict(field_values) 
                self.permute_item(pick, new_line, next_args,
                                  permute_field_values)
                if next_args == None:
                    self.collect_complete_command(line, field_values)
            elif type(pick) == tuple:
                in_order_values = None
                if field_values != None:
                    in_order_values = dict(field_values)
    ...
    def permute_in_order(self, items, line, next_args, field_values):
        ...
        if len(items):
            ...
            token = False
            if type(item) == tuple:
                self.permute_in_order(item, line, list(next_args), field_values)
            elif type(item) == dict:
                self.permute_item(item, list(line), list(next_args), field_values)
            else:
    ...
    def permute_command_common(self, command):
        ...
        # if the first item is a dict, then there's a needing populattion
        # from field_values.
        if type(command.get('name')) == dict:
            if self.qualify or (field_values != None):
                # the call to field_to_generator will correctly remove
                # a field from field_values, but this will break the
                # caller's assumption that this dict() is preserved.
                # Make a copy, then remove the name, and use that
                # copy for all subsequent calls.
                if field_values != None:
                    field_values = dict(field_values)
                ...
                if name == None:
                    if debug.description():   # description debugging
                        print 'broken command, missing', command['name']['field']
                    return
        ...
        # the command may have 'dict' items.  these items need to be subtracted from
        # the field_values since the command won't directly mention these fields, but
        # when they appear as 'data' items, they get populated.
        if field_values != None:
            data = command.get('data')
            if data:
                remove_names = []
                for (n,v) in data.items():
                    if n not in field_values:
                        return
                    if str(field_values[n]) == str(v):
                        remove_names.append(n)
                    # leave values which differ from the field values alone,
                    # if the command variation can't configure them,
                    # then that variation ought to be dropped.
                field_values = dict([[n, v] for (n,v) in field_values.items()
                                     if n not in remove_names])

        if type(args) == tuple:
            self.permute_in_order(args, line, [], field_values)
        elif type(args) == dict:
            self.permute_item(args, line, [], field_values)
        else:
            print 'PC ', type(args)
    ...
    def permute_command(self, command, config = True, un_config = True):
        if debug.description():   # description debugging
            print 'PERMUTE', command['self']

        if config:
            self.is_no_command = False
            result = self.permute_command_common(command)

        if is_no_command_supported(command) and un_config:
...
def permute_command(words, qualify):
    ...
    permute BIG_TAP_BIGTAP_SERVICE_COMMAND_DESCRIPTION
    """
    if len(words) == 0:
...
def _add_applicable_modes(command, mode_dict):
    ...
    """
    description_feature = command.get('feature')
    if description_feature:
        if not feature.feature_enabled(description_feature):
            return
...
def init_command(bs):
    # FIXME HACK: bigsh global to access top-level CLI object
    global bigsh
    bigsh = bs

    c_actions.init_actions(bs)
    c_data_handlers.init_data_handlers(bs)
    c_completions.init_completions(bs)
    c_validations.init_validations(bs)
    ...
    add_typedef({
        'name'       : 'controller',
        'help-name'  : 'Controller node (not alias nor uuid)',
        'base-type'  : 'string',
        'validation' : 'validate-controller-node',
    })
