...
class OFChannelHandler
    ...
    private long lastAssertTime = 0;
    private long assertTimeInterval = 1000;
    ...
    private class RoleChanger {
        ...
        synchronized void sendRoleRequestIfNotPending(Role role)
            ...
            if (now - lastAssertTime < assertTimeInterval) {
                return;
            }

            lastAssertTime = now;
            if (assertTimeInterval < 120000) { // 2 minutes max
                assertTimeInterval <<= 1;
    ...
    protected Query getCoreSwitchQuery(String dpid) throws BigDBException {
        return Query.builder()
                .setBasePath("/core/switch[dpid=$dpid]/core-switch")
                .setVariable("dpid", dpid).getQuery();
    ...
    private void readPropertyFromStorage() {
        try {
            Query q = getCoreSwitchQuery(sw.getStringId());
            Treespace ts = 
                    this.controller.getBigDBService().getControllerTreespace();
            DataNode d = 
                    ts.queryData(q, AuthContext.SYSTEM).getSingleDataNode();
            boolean is_core_switch = d.getBoolean(false);
            if (log.isDebugEnabled()) {
                log.debug("Read core-switch " +
            ...
            log.error("Failed to read core-switch property for switch {}",
                      sw.getStringId(), e);
        }
    }
