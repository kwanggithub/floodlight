...
import org.projectfloodlight.flowcache.PriorityPendingQueue.EventPriority;
import org.projectfloodlight.linkdiscovery.ILinkDiscovery;
...
public class PortDownReconciliation implements IFloodlightModule,
    ...
    protected final static Logger log = 
            LoggerFactory.getLogger(PortDownReconciliation.class);
    ...
    protected List<OFFlowStatisticsReply> statsReply;
     ...
     */
    @Override
    public org.projectfloodlight.core.IListener.Command
        ...
        if (lds != null) {
            ...
            for (OFMatchReconcile ofmr : ofmRcList) {
                ...
                if (ofmr.rcAction.equals(OFMatchReconcile.ReconcileAction.UPDATE_PATH)) {
                    ...
                    // issues
                    for (Entry<IOFSwitch, Map<Short, List<OFMatch>>> entry : 
                            neighborSwitches.entrySet()) {
                        IOFSwitch neighborSwitch = entry.getKey();
    ...
    public void deleteInvalidFlows(IOFSwitch sw,
        ...
        for (Entry<Short,List<OFMatch>> entry : invalidOutportAndMatch.entrySet()) {
            Short outPort = entry.getKey();
            ...
            for (OFFlowStatisticsReply flow : flows) {
                ...
                for (OFMatch match : entry.getValue()) {
            ...
            // invalid matches
            for (Entry<IOFSwitch, Map<Short, List<OFMatch>>> entry2 :
                    neighborSwitches.entrySet()) {
                IOFSwitch neighborSwitch = entry2.getKey();
