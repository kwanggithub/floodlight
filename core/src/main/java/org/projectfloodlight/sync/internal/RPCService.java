...
public class RPCService {
    ...
    /**
     * The cluster manager for this node
     */
        ...
        if (!mw.disconnected && 
            ...
            try {
                while (!mw.disconnected && 
                    ...
                    long towait = now - start;
                    while ((towait = mw.full.awaitNanos(towait)) > 0);
                }
            } finally {
                mw.lock.unlock();
            }
    ...
    protected class ConnectCFListener implements ChannelFutureListener {
        ...
        public ConnectCFListener(ClusterNode node) {
            super();
            this.node = node;
        }
